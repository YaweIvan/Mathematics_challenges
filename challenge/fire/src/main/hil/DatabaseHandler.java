...
public class DatabaseHandler {
    ...
    protected static void handleLogin(String loginRequest) {
        ...
        if (parts.length >= 3) {
            ...
            String sql = "SELECT * FROM schools WHERE repEmail = ? AND School_Registration_Number  = ? ";
    ...
    protected static void Challenges() {
        String query = "SELECT Challenge_ID, Title, Start_Date, End_Date FROM challenge";
        try(Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD);
    ...
    protected static boolean isChallengeOpen(String challengeTitle) throws SQLException {
    ...
    protected static boolean recordAttempt(String schoolRegistrationNumber, int score, long totalChallengeTime) throws SQLException {
        String query = "INSERT INTO Attempts (Attempt_ID,Challenge_ID, Score, Time_Taken,School_Registration_Number) VALUES (?, ?, ?)";
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD);
             PreparedStatement statement = conn.prepareStatement(query);){
            statement.setString(1, schoolRegistrationNumber);
            statement.setBigDecimal(2, BigDecimal.valueOf(score));
            statement.setInt(3, (int) (totalChallengeTime / 1000));
